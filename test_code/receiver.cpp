...
void readMem(ImageBatch msg) {
  ...
  for (int i = 0; i < msg.num_images; i++) {
    ...
    memcpy(metadata_buffer, &local_data[offset + sizeof(metadata_size)],
           metadata_size);
    Metadata metadata;
    ...
    memcpy(image_buffer,
           &local_data[offset + sizeof(metadata_size) + metadata_size],
           metadata.size());
    offset += sizeof(metadata_size) + metadata_size + metadata.size();

    // cv::Mat rawImage(metadata.height(), metadata.width(), CV_16UC1,
    // image_buffer); cv::Mat demosaicedImage; cv::cvtColor(rawImage,
    // demosaicedImage, cv::COLOR_BayerGR2RGB); demosaicedImage *= 16;

    // save to path
    fs::path dir("./");
    fs::path file("image_" + std::to_string(i) + ".raw");
    ...
    outFile.write(reinterpret_cast<const char *>(image_buffer),
                  metadata.size());
...
int main() {
  ...
  while (true) {
    ...
    if ((msgQueueId = msgget(71, 0644)) == -1 ||
        msgrcv(msgQueueId, &message, sizeof(message), 1, 0) == -1) {
      continue;
    }

    cout << "Receiver: Message received: " << message.batch_size << " "
         << message.shm_key << endl;
    readMem(message);
  }
