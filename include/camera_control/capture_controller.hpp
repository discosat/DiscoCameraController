...
public:
    ...
    static void CaptureCallback(char *camera_id, uint8_t camera_type, uint32_t exposure, double iso, uint32_t num_images, uint32_t interval, uint32_t obid, uint32_t pipeline_id, uint32_t max_processing_latency, void *obj, u_int16_t *error)
    {
        if (obj)
        {

            CaptureMessage msg = CreateCaptureMessage(camera_id, camera_type, exposure, iso, num_images, interval, obid, pipeline_id, max_processing_latency);
    ...
    static CaptureMessage CreateCaptureMessage(char *camera_id, uint8_t camera_type, uint32_t exposure, double iso, uint32_t num_images, uint32_t interval, uint32_t obid, uint32_t pipeline_id, uint32_t max_processing_latency)
    {
        ...
        message.OBID = obid;
        message.MaxProcessingLatency = max_processing_latency;
